...
def post_process(
    ...
    combined_heatmap = combine_heatmaps(clothing_type, hm_pred, hm_pred2, info)
    ...
    return np.stack([x, y, np.ones(x.shape)], axis=1).astype(np.int16)
...
def combine_heatmaps(
    ...
    """Undo the horizontal flip and combine the two model predictions."""
    ...
    return hm_pred + flipped


def predict_keypoints(clothing_type: str, img_bgr: np.ndarray, net) -> np.ndarray:
    keypoints, _, _ = predict_keypoints_with_heatmaps(clothing_type, img_bgr, net)
    return keypoints


def predict_keypoints_with_heatmaps(
    clothing_type: str,
    img_bgr: np.ndarray,
    net,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Return decoded keypoints, combined heatmaps, and per-keypoint peak scores."""
    ...
    combined_heatmap = combine_heatmaps(clothing_type, hm_pred, hm_pred2, info)
    keypoints = post_process(clothing_type, hm_pred, hm_pred2, info)
    scores = np.max(
        combined_heatmap.reshape(combined_heatmap.shape[0], -1),
        axis=1,
    )
    return keypoints, combined_heatmap, scores
...
def keypoints_to_json(
    ...
    h, w = image_shape[:2]
    points = []
    for name, (u, v, visible) in zip(KEYPOINT_NAMES[clothing_type], keypoints.tolist()):
        points.append(
            {
                "name": name,
                "u": int(u),
...
def save_result(
    ...
    payload = keypoints_to_json(clothing_type, keypoints, f"{stem}.png", frame_bgr.shape, elapsed_ms)
...
def process_image_paths(args: argparse.Namespace, net: ailia.Net, dirs: dict[str, Path]) -> int:
    ...
    for i, path in enumerate(paths, start=1):
        ...
        keypoints = predict_keypoints(args.clothing_type, frame, net)
